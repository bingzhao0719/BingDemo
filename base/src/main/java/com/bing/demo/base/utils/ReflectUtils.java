...
public class ReflectUtils {
     ...
     * @return
     */
    public static Field getField(Class<?> sourceClass,
                                 String fieldName,
                                 boolean isFindDeclaredField,
                                 boolean isUpwardFind) {
        Field field = null;
        try {
            field = isFindDeclaredField
        ...
        if (isGetParentField) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Field field : isGetDeclaredField
     ...
     */
    public static boolean setField(Object object,
                                   String fieldName,
                                   Object newValue,
                                   boolean isFindDeclaredField,
                                   boolean isUpwardFind) {
    ...
    public static Method getMethod(Class<?> sourceClass,
        ...
        Method method = null;
        try {
            method = isFindDeclaredMethod
                     ? sourceClass.getDeclaredMethod(methodName,
            ...
            if (isUpwardFind) {
                ...
                while (method == null && classs != null) {
                    try {
                        method = isFindDeclaredMethod
                                 ? classs.getDeclaredMethod(methodName,
                                                            methodParameterTypes)
     ...
     * @return 给定类的所有方法
     */
    public static List<Method> getMethods(Class<?> clas,
                                          boolean isGetDeclaredMethod,
        ...
        if (isFromSuperClassGet) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Method method : isGetDeclaredMethod
    ...
    public static Constructor<?> getConstructor(Class<?> sourceClass,
        ...
        try {
            method = isFindDeclaredConstructor
    ...
    public static List<Constructor<?>> getConstructors(Class<?> sourceClass,
        ...
        if (isFromSuperClassGet) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Constructor<?> constructor : isFindDeclaredConstructor
                                                      ? classList.get(w)
